...
def evaluate(num_episodes=20):
    ...
    model = PPO.load("sb3_ppo_cityflow")
    ...
    for ep in range(1, num_episodes + 1):
        ...
        print(f"[SB3 PPO EVAL] Episode {ep}/{num_episodes} total_reward={total_reward:.2f}")

    avg_reward = float(np.mean(episode_rewards))
    print(f"\nAverage SB3 PPO eval reward over {num_episodes} episodes: {avg_reward:.2f}")
